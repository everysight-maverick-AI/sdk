...
def enc_RLE(binData, idx_out):
    ...
    while idx < idx_out:
        b = binData[idx]
        idx += 1
        #print("char={:02X}".format(b))
        if prevByte != b or currRunCount >= 128:
            if currRunCount > 1:
                if len(rawBuff) > 1:
                    #print("RAW count={} data={}".format(len(rawBuff) - 1, bytes(rawBuff[:len(rawBuff) - 1]).hex()))
                    encData.append(len(rawBuff) - 2)
                    encData += rawBuff[:len(rawBuff) - 1]
                    rawBuff = []
                    
                #print("RLE count={} byte={:02X}".format(currRunCount, prevByte))
                encData.append(0x80 | (currRunCount - 1))
                encData.append(prevByte)
                rawBuff = []
    ...
    if len(rawBuff) > 0:
        #print("RAW count={} data={}".format(len(rawBuff), bytes(rawBuff).hex()))
        encData.append(len(rawBuff) - 1)
        encData += rawBuff
...
#######################################################################
parser = argparse.ArgumentParser(description='Converting TrueType/OpenType/WOF Fonts to SIF2 (System Independent Font)')
...
parser.add_argument('--bpp', type=int, choices=[1, 2, 4, 8], default=2, help='Bit per pixel')
...
#parser.add_argument('--rle', action='store_true', help='Use RLE encoding')
...
total_headers_size = 8 + 4*len(char_ranges)
for r in char_ranges:
    for c in range(r[0], r[1] + 1):
        total_headers_size += 8
...
for r in char_ranges:
    ...
    for c in range(r[0], r[1] + 1):
        ...
        if w > 0 and h > 0:
            ...
            # encode RLE
            if use_rle:
                encData = enc_RLE(binData, idx_out)
                # should we use RLE for this char?
                if len(encData) < len(binData):
                    #print("encData={}".format(bytes(encData).hex(",")))
                    binData = np.array(encData, dtype='ubyte')
                    rleUsed = 1
        ...
        if verbose:
            print("0x{:04X}\t({})\tsize: {}:{}\tshift: {}:{}\tinc: {}\t\t{} bytes\t rle={}".format(c, chr(c), w, h, left, top, advance, bsize, rleUsed))

        # pad to 16 bytes
        pad = 0
        offset = total_headers_size + total_binary_size
        if offset % 8 != 0:
            pad = 8 - offset % 8
            binData = np.concatenate((np.zeros(pad, dtype='ubyte'), binData), axis=None)

        range_data.append((c, w, h, left, top, advance, offset + pad, binData, rleUsed))
        total_binary_size += len(binData)
        x += advance
...
fontsXDict = {}
# write header
binfile = open(out_file_path + ".sif2", "wb")

# SIF2 format (all values LSB)
binfile.write(pack("BBBB", ord('S'), ord('I'), ord('F'), ord('2')))
binfile.write(pack("<B", max_height))
binfile.write(pack("<B", 0))    # placeholder
binfile.write(pack("<B", bit_per_pixel))
binfile.write(pack("<B", len(char_ranges)))

#  write range data
for r in char_ranges:
    binfile.write(pack("<H", r[0]))
    binfile.write(pack("<H", r[1]))
...
for i in range(0, len(char_ranges)):
    ...
    for i in range(0, r[1] - r[0] + 1):
        c, w, h, left, top, advance, binary_offset, data, rleUsed = d[i]
        binfile.write(pack("<B", w))
        binfile.write(pack("<B", h))
        binfile.write(pack("<b", left))
        binfile.write(pack("<b", max_height - (baseline + top)))
        binfile.write(pack("<B", advance))
        binfile.write(pack("<B", rleUsed))
        binfile.write(pack("<H", binary_offset))
        fontsXDict[c]=advance

# write binay data
for i in range(0, len(char_ranges)):
    for c, _, _, _, _, _, _, data,_ in output_data[i]:
        if len(data) > 0:
            binfile.write(data.tobytes())

totalSize = binfile.tell()
if totalSize % 16 != 0:
    pad = 16 - totalSize % 16
    binfile.write(np.zeros(pad, dtype='ubyte'))

ovrallSize = binfile.tell()
if ovrallSize > MAX_DATA_SIZE:
    print("Font is too large for the glasses size={} max={}".format(ovrallSize, MAX_DATA_SIZE))
...
# compress to zlib
#fd = open(out_file_path + ".sif2", "rb")
#bindata = fd.read()
#fd.close()
#fd = open(out_file_path + ".sif2z", "wb")
